...
<template>
  ...
  <!-- 자식 컴포넌트가 emit('count-increment') 를 호출하면 부모의 countIncrement 실행 -->
  <!-- 자식 컴포넌트가 emit('state-count-increment') 를 호출하면 부모의 stateCountIncrement 실행 -->
  <DefineEmits
...
<!--  DefineProps 컴포넌트에서는 부모(App.vue)에서 전달한 반응형 데이터 defineProps()
  사용하여 받아서 출력할 수 있다. -->
