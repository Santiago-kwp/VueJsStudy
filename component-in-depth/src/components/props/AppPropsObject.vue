...
const fruits = reactive([
  { id: 'f1', name: '사과', checked: true, price: 2000, origin: '한국' },
  {
    id: 'f2',
    name: '파인애플',
    checked: false,
  ...
  { id: 'f3', name: '포도', checked: false, price: 3000, origin: '한국' },
  { id: 'f4', name: '딸기', checked: true, price: 4000, origin: '제주' },
  {
    id: 'f5',
    name: '아보카도',
    checked: false,
  ...
  { id: 'f6', name: '메론', checked: false, price: 6000, origin: '태국' },
]);

const countChecked = computed(
  () => fruits.filter((fruit) => fruit.checked).length
);

const showFruitName = (name) => {
  alert(`선택한 과일 : ${name}`);
};
</script>
  ...
  <div>
    ...
    <div class="fruits">
      ...
      <AppPropsObjectChildDestructure
        ...
        :key="fruit.id"
        :fruit="fruit"
        @show-fruit-name="showFruitName"
      />
      <p>{{ countChecked }}개가 선택되었습니다.</p>
    </div>
  </div>
</template>
